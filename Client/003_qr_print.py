...
3. Sends image to WebSocket server, receives receipt, prints it
4. Auto-reconnects if WebSocket fails
5. Cooldown between captures to prevent spam
"""
...
async def ws_send_image(image_path, ws_server):
    """Send image to server with auto-reconnect."""
    attempt = 0
    while True:
        try:
...
def trigger_capture():
    """Trigger a capture via HTTP."""
    global capture_triggered, last_capture_time

    # Optional QR code parameter (for logging)
